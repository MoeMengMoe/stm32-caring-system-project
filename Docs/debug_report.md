...
## 2026-05-23：检测层传感器全部验证

- BME280：温度、湿度、气压串口日志正常。
- MQ 气体模块：`AO -> 2k/3.3k 分压 -> A2 / PC3 / ADC1_IN4` 采样正常。
- PIR：`A3 / PB0 / PIR_IN` 人体触发验证正常。
- Rd-03 V2：`A4 / PC1 / RD03_OUT` 人体存在触发验证正常。
- 结论：MVP v1 检测层的环境、气体、人体活动/存在输入均已完成基础上板闭环；下一阶段可进入风险状态机和告警/上报链路。
