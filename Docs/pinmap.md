...
| MQ 气体模块 | ADC1 | A0 / PA0 | AO | `ADC1_IN5`；烟雾/可燃气体模拟量；MVP 必须使用 AO，不只用 DO |
| 蜂鸣器 | GPIO | PB5 | Output | 本地告警；检测模块完成后再接入 |
| LED / LD1 绿灯 | GPIO | PC7 | Output | NUCLEO-U5A5ZJ-Q 板载 LD1 默认连接；PA5 仅为改焊桥后的可选连接 |
| PIR | GPIO | A1 / PA1 | Input | `PIR_IN`，下拉输入；人体活动检测 |
| Rd-03 V2 | GPIO | A2 / PA4 | Input | `RD03_OUT`，下拉输入；人体存在数字状态 |
...
- PIR 不再默认占用 `PC13`，因为该脚可能与板载用户按钮或低功耗唤醒语义混淆；MVP v1 使用 Arduino `A1 / PA1`。
- MQ 模块由面包板电源模块供 `5V`，`AO` 通过 `2k/3.3k` 分压后接入 `A0`，日志中 `mq_adc_mv` 是 A0 电压，`mq_ao_est_mv` 是反推的 MQ AO 电压。
