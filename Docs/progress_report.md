...
它已经能够采集环境和人体存在相关数据，也已经具备将状态经 ESP8266、MQTT 发送到 Home Assistant 的代码与服务器基础；本地 SOS/ACK 输入和蜂鸣器告警第一版已完成上板验收，但真实传感器数据的完整联网闭环、正式风险判断和通知联动仍需继续验收。
...
| 气体模块模拟量监测 | MQ 模块 AO | 读取 AO 分压后的 ADC 数值，保留 mV 调试链路，并估算 ppm 用于屏幕和联网展示 | ppm 为经验曲线估算值，未经过标准气体标定 |
...
| 调试信息查看与兜底控制 | USART1 / ST-LINK VCP | 在 COM6 查看启动、采集、事件和发送日志，也可用 `s/a/c/1/2/o/n/p/h` 触发演示命令 | 主要用于开发调试和现场排障 |
...
| `gas` | MQ-2 烟雾/可燃气等效 ppm 估算值，用于 Home Assistant 和屏幕直观显示 |
...
- STM32 主控 `Modules/app` 业务层第一版：支持 dashboard `D` 命令触发场景、用户确认、清除、模拟断网/恢复、雷达久静触发、确认超时升级和 `E` 事件帧上报。
这些功能需要一次完整的上板验收，并在 `Docs/debug_report.md` 中留下日志或截图记录。

### 4.3 尚未实现

- 经过上板验收的正式看护风险状态机和事件码；当前已有代码第一版，但还未完成现场闭环验证。
- 完整的基于持续时间的异常判断，例如气体持续升高；雷达久静触发第一版已接入，仍需按实际安装位置调阈值。
- MQ 模块标定、气体浓度或可靠告警阈值。
...
- MQTT `event`、`alarm` 独立事件上报的完整上板链路；STM32 `E` 事件帧发送代码已接入，待 ESP8266/MQTT/面板联调验收。
...
5. 增加 MQTT 事件上报和 Home Assistant 自动化，让状态变化真正触发告警展示。
6. 继续整理实体按钮机械接法，当前第一版使用短接 `D0/D1 -> GND` 和 COM6 控制入口作为稳定验收手段。
